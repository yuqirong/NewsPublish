	...
	$(function() {
		$("#btn").click(
				function() {
					...
					var city = $("#s2").val();
					var county = $("#s3").val();
					var u = "WeatherAction_getWeather";
					$
							.ajax({
								type : "post",
								url : u,
								...
								success : function(msg) {
									$("#res").remove();
									var $Result = $(msg);
									var area = $Result.find("string:eq(1)")
											.text();
									var jweather = $Result.find("string:eq(7)")
											.text();
									var jtemperature = $Result.find(
											"string:eq(8)").text();
									var mweather = $Result
											.find("string:eq(12)").text();
									var mtemperature = $Result.find(
											"string:eq(13)").text();
									var hweather = $Result
											.find("string:eq(17)").text();
									var htemperature = $Result.find(
											"string:eq(18)").text();
									$("#weather").append(
											area + "&nbsp;<br>" + jweather
													+ "&nbsp;" + jtemperature
...
<body onload="setup()">
	<s:if test="#session.weather == null">
		<div id="res">
			天气预报： <select id="s1" name="province"><option>省份</option></select>&nbsp;
			<select id="s2" name="city"><option>地级市</option></select>&nbsp; <select
				id="s3" name="county"><option>市、县级市、县</option></select>&nbsp;
			<s:submit name="submit" value="确定" id="btn" cssClass="right-button01" />
		</div>
	</s:if>
	<div id="weather">
		<s:if test="#session.weather != null">
			<s:property value="#session.weather.county" />
			&nbsp;<br>
			<s:property value="#session.weather.jweather" />
			<s:property value="#session.weather.jtemperature" />
			&nbsp;<br>
			<s:property value="#session.weather.mweather" />
			<s:property value="#session.weather.mtemperature" />
			&nbsp;<br>
			<s:property value="#session.weather.hweather" />
			<s:property value="#session.weather.htemperature" />
		</s:if>
	</div>
</body>
</html>